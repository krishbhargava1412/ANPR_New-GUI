...
const SettingsPage = (() => {
    ...
    function syncSelectedModel() {
        ...
        const savedModel = currentSettings?.model_name;
        if (!select || !savedModel) return;
        const hasOption = Array.from(select.options).some(o => o.value === savedModel);
        if (hasOption) select.value = savedModel;
    }
    ...
    async function save() {
        try { currentSettings = await API.put('/api/settings', {
        ...
        }); syncSelectedModel(); alert('Settings saved'); } catch(e) { alert('Failed: '+e.message); }
    }
